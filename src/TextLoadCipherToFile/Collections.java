...
public class Collections {
    ...
    public static void main (String[] args) {
        ...
        //sorted
        SortedSet<TextLoadCipherToFile.Instrument> myInstruments = new TreeSet<> ();

        myInstruments.add (new Piano ("black", 2));
        myInstruments.add (new TextLoadCipherToFile.Gitar ( "acustic", 5));
        myInstruments.add (new Tromba ("gold", 7));

        int i = 0;
        PrintStream printf = System.out.printf ("%6s %30s %10S\n", "number", "name", "price");
        for (TextLoadCipherToFile.Instrument instrument : myInstruments) {
